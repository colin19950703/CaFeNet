...
def create_dataset(
    ...
    In parenthesis after each arg are the type of data supported for each arg, one of:
    ...
    Args:
        name: data name, empty is okay for folder based datasets
        root: root folder of data (all)
        split: data split (all)
        search_split: search for split specific child fold from root so one can specify
            `imagenet/` instead of `/imagenet/val`, etc on cmd line / config. (folder, torch/folder)
        class_map: specify class -> index mapping via text file or dict (folder)
        load_bytes: load data, return images as undecoded bytes (folder)
        download: download data if not present and supported (HFDS, TFDS, torch)
        is_training: create data in train mode, this is different from the split.
        ...
        repeats: data repeats per iteration i.e. epoch (TFDS, WDS)
        **kwargs: other args to pass to data
    ...
    if name.startswith('torch/'):
        ...
        if name in _TORCH_BASIC_DS:
            ...
            assert has_places365, 'Please update to a newer PyTorch and torchvision for Places365 data.'
            if split in _TRAIN_SYNONYM:
            ...
            assert has_qmnist, 'Please update to a newer PyTorch and torchvision for QMNIST data.'
            use_train = split in _TRAIN_SYNONYM
            ds = QMNIST(train=use_train, **torch_kwargs)
        elif name == 'imagenet':
            assert has_imagenet, 'Please update to a newer PyTorch and torchvision for ImageNet data.'
            ...
            assert False, f"Unknown torchvision data {name}"
