...
try:
    ...
    # NOTE uncomment below if having file limit issues on data build (or alter your OS defaults)
    # import resource
...
class ReaderTfds(Reader):
    ...
    There several things to be aware of:
      ...
      * The sharding (splitting of data into TFRecord) files imposes limitations on the number of
      ...
      * This wrapper is currently configured to return individual, decompressed image samples from the TFDS
        data. The augmentation (transforms) and batching is still done in PyTorch. It would be possible
    ...
    def __init__(
        ...
        Args:
            root: root data dir (ie your TFDS_DATA_DIR. not data specific sub-dir)
            name: tfds data name (eg `imagenet2012`)
            split: tfds data split (can use all TFDS split strings eg `train[:10%]`)
            is_training: training mode, shuffle enabled, data len rounded by batch_size
            batch_size: batch_size to use to unsure total samples % batch_size == 0 in training across all dis nodes
            download: download and build TFDS data if set, otherwise must use tfds CLI
            repeats: iterate through (repeat) the data this many times per iteration (once if 0 or 1)
    ...
    def _lazy_init(self):
        ...
        This is necessary to init the Tensorflow data pipeline in the (dataloader) process that
        will be using the data instance. The __init__ method is called on the main process,
        this will be called in a dataloader worker process.

        NOTE: There will be problems if you try to re-use this data across different loader/worker
        instances once it has been initialized. Do not call any data methods that can call _lazy_init
        before it is passed to dataloader.
        """
        worker_info = torch.utils.data.get_worker_info()

        # setup input context to split data across distributed processes
        num_workers = 1
        global_worker_id = 0
        if worker_info is not None:
            ...
            in data < total number of workers. Otherwise sub-split API is used for datasets without enough shards or
            ...
            if should_subsplit:
                # split the data w/o using sharding for more even samples / worker, can result in less optimal
    ...
    def filenames(self, basename=False, absolute=False):
        """ Return all filenames in data, overrides base"""
        if self.ds is None:
            self._lazy_init()
        names = []
        for sample in self.ds:
