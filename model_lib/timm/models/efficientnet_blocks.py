...
class FBNetBlock(nn.Module):
    def __init__(
            self, in_chs, out_chs, dw_kernel_size=3, stride=1, dilation=1, group_size=1, pad_type='',
            noskip=False, exp_ratio=1.0, exp_kernel_size=1, pw_kernel_size=1, act_layer=nn.ReLU,
            num_group=1, se_layer=None, conv_kwargs=None, drop_path_rate=0.):
        super(FBNetBlock, self).__init__()
        if num_group == 1:
            groupnorm_act_layer = get_norm_act_layer(nn.GroupNorm, act_layer, num_group)
        else:
            groupnorm_act_layer = get_norm_act_layer(nn.GroupNorm, act_layer, num_group)
        batchnorm_act_layer = get_norm_act_layer(nn.BatchNorm2d, act_layer)
        conv_kwargs = conv_kwargs or {}
        mid_chs = make_divisible(in_chs * exp_ratio)
        groups = num_groups(group_size, mid_chs)
        self.has_skip = (in_chs == out_chs and stride == 1) and not noskip
        ...
        # Point-wise expansion
        self.conv_pw = create_conv2d(in_chs, mid_chs, exp_kernel_size, padding=pad_type, **conv_kwargs)
        self.bn1 = groupnorm_act_layer(mid_chs, inplace=True)
        # Depth-wise convolution
        self.conv_dw = create_conv2d(
            mid_chs, mid_chs, dw_kernel_size, stride=stride, dilation=dilation,
            groups=groups, padding=pad_type, **conv_kwargs)
        self.bn2 = batchnorm_act_layer(mid_chs, inplace=True)
        ...
        # Point-wise linear projection
        self.conv_pwl = create_conv2d(mid_chs, out_chs, pw_kernel_size, padding=pad_type, **conv_kwargs)
        self.bn3 = groupnorm_act_layer(out_chs, apply_act=False)
        self.drop_path = DropPath(drop_path_rate) if drop_path_rate else nn.Identity()
        self.shuffle = ChannelShuffle(num_group)
    def feature_info(self, location):
        if location == 'expansion':  # after SE, input to PWL
            return dict(module='conv_pwl', hook_type='forward_pre', num_chs=self.conv_pwl.in_channels)
        else:  # location == 'bottleneck', block output
    ...
    def forward(self, x):
        shortcut = x
        x = self.conv_pw(x)
        x = self.shuffle(self.bn1(x))
        x = self.conv_dw(x)
        x = self.bn2(x)
        x = self.se(x)
        x = self.conv_pwl(x)
        x = self.shuffle(self.bn3(x))
        if self.has_skip:
            x = self.drop_path(x) + shortcut
        return x

class FBNetBlock5to3(nn.Module):
    def __init__(
            self, in_chs, out_chs, dw_kernel_size=3, stride=1, dilation=1, group_size=1, pad_type='',
            noskip=False, exp_ratio=1.0, exp_kernel_size=1, pw_kernel_size=1, act_layer=nn.ReLU,
            num_group=1, se_layer=None, conv_kwargs=None, drop_path_rate=0.):
        super(FBNetBlock5to3, self).__init__()
        if num_group == 1:
            groupnorm_act_layer = get_norm_act_layer(nn.GroupNorm, act_layer, num_group)
        else:
            groupnorm_act_layer = get_norm_act_layer(nn.GroupNorm, act_layer, num_group)
        ...
        self.bn1 = groupnorm_act_layer(mid_chs, inplace=True)
        # Depth-wise convolution
        self.conv_dw_1 = create_conv2d(
            mid_chs, mid_chs, dw_kernel_size, stride=stride, dilation=dilation,
            groups=groups, padding=pad_type, **conv_kwargs)
        self.bn2 = batchnorm_act_layer(mid_chs, inplace=True)
        self.conv_dw_2 = create_conv2d(
            mid_chs, mid_chs, dw_kernel_size, stride=stride, dilation=dilation,
            groups=groups, padding=pad_type, **conv_kwargs)
        self.bn3 = batchnorm_act_layer(mid_chs, inplace=True)
        # Squeeze-and-excitation
        self.se = se_layer(mid_chs, act_layer=act_layer) if se_layer else nn.Identity()

        # Point-wise linear projection
        self.conv_pwl = create_conv2d(mid_chs, out_chs, pw_kernel_size, padding=pad_type, **conv_kwargs)
        self.bn4 = groupnorm_act_layer(out_chs, apply_act=False)
        self.drop_path = DropPath(drop_path_rate) if drop_path_rate else nn.Identity()
        self.shuffle = ChannelShuffle(num_group)
    def feature_info(self, location):
        if location == 'expansion':  # after SE, input to PWL
